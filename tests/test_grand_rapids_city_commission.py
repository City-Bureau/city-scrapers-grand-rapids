...
from city_scrapers.spiders.grand_rapids_city_commission import (
    GrandRapidsCityCommissionSpider,
)

test_response = file_response(
    join(dirname(__file__), "files", "grand_rapids_city_commission.html"),
    url="http://grandrapidscitymi.iqm2.com/Citizens/calendar.aspx?View=List",
)
spider = GrandRapidsCityCommissionSpider()
...
def test_id():
    assert (
        parsed_items[0]["id"]
        == "grand_rapids_city_commission/202301101400/x/city_commission"
    )

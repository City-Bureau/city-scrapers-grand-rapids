...
from city_scrapers.spiders.grand_rapids_officers_compensation import (
    GrandRapidsOfficersCompensationSpider,
)

test_response = file_response(
    join(dirname(__file__), "files", "grand_rapids_officers_compensation.html"),
    url="http://grandrapidscitymi.iqm2.com/Citizens/calendar.aspx?View=List",
)
spider = GrandRapidsOfficersCompensationSpider()
...
def test_id():
    assert (
        parsed_items[0]["id"]
        == "grand_rapids_officers_compensation/202303131500/x/local_officers_compensation_commission"  # noqa
    )

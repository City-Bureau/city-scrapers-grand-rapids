...
class GrandRapidsBrownfieldRedevelopmentAgencySpider(CityScrapersSpider):
    name = "grand_rapids_brownfield_redevelopment_agency"
    agency = "Grand Rapids Brownfield Redevelopment Agency"
    ...
    def parse(self, response):
        ...
        for item in response.css(".MeetingRow"):
            if item.css(".RowTop .RowRight span::text").get() != "Cancelled":
                if "Brownfield" in item.css(".RowBottom div:nth-child(2)::text").get():
                    meeting = Meeting(
    ...
    def _parse_title(self, item):
        ...
        title = (
            item.css(".RowBottom div:nth-child(2)::text").get().split("-")[0].strip()
        )
        return title
